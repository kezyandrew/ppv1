...
function required()
{
    ...
    $public_routes = array("frontend", "payu", "status", "cronjobs", "request", "auth", "site", "api");
    $is_public_route = in_array($RTR->class, $public_routes);
    ...
    if (!$is_public_route) {
        ...
        // Initialize hospital_id only for authenticated routes
        ...
        if ($CI->ion_auth->in_group(array('superadmin'))) {
            ...
            } elseif (in_array($group_name, array('doctor', 'patient', 'nurse', 'pharmacist', 'laboratorist', 'accountant'))) {
                $CI->hospital_id = $CI->db->get_where($group_name, array('ion_user_id' => $current_user_id))->row()->hospital_id;
            } else {
                // Default to getting hospital_id from session if set
                $CI->hospital_id = $CI->session->userdata('hospital_id');
                if (empty($CI->hospital_id)) {
                    // Fallback to getting from hospital table if user is associated with a hospital
                    $hospital_data = $CI->db->get_where('hospital', array('ion_user_id' => $current_user_id))->row();
                    if ($hospital_data) {
                        $CI->hospital_id = $hospital_data->id;
                    }
                }
            }
            // Store in session
