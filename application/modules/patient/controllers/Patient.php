...
class Patient extends MX_Controller
{
                                                    ...
                                                    <div class="card col-md-6">
                                                        <div class="card-body">
                                                            ...
                                                            <p></p>
                                                             <i class=" fa fa-user-md"></i>
                                                                <h4>' . $lab_doctor . '</h4>
                                                                    <a class="btn btn-sm invoicebutton" title="Lab" style="color: #fff;" href="lab/invoice?id=' . $lab->id . '"><i class="fa fa-file-text"></i>' . lang('report') . '</a>
                                                        </div>
                                                    </div> 
                                                </div>
                                            </div>
    ...
    function getPatient()
    {
        ...
        foreach ($data['patients'] as $patient) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                $options1 = '<a type="button" class="btn btn-primary editbutton btn-sm mb-1" title="' . lang('edit') . '" data-toggle="modal" data-id="' . $patient->id . '"><i class="fa fa-edit"></i> ' . lang('edit') . '</a>';
            }

            $options2 = '<a class="btn btn-info detailsbutton btn-sm mb-1" title="' . lang('info') . '" href="patient/patientDetails?id=' . $patient->id . '"><i class="fa fa-info"></i> ' . lang('info') . '</a>';

            $options3 = '<a class="btn btn-success btn-sm mb-1" title="' . lang('history') . '" href="patient/medicalHistory?id=' . $patient->id . '"><i class="fa fa-stethoscope"></i> ' . lang('history') . '</a>';

            $options4 = '<a class="btn btn-secondary invoicebutton btn-sm mb-1" title="' . lang('payment') . '" href="finance/patientPaymentHistory?patient=' . $patient->id . '"><i class="fa fa-money-bill-alt"></i> ' . lang('payment') . '</a>';

            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                $options5 = '<a class="btn btn-danger delete_button btn-sm mb-1" title="' . lang('delete') . '" href="patient/delete?id=' . $patient->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> ' . lang('delete') . '</a>';
            }

            $options6 = '<a type="button" class="btn btn-warning detailsbutton inffo btn-sm mb-1" title="' . lang('info') . '" data-toggle="modal" data-id="' . $patient->id . '"><i class="fa fa-info"></i> ' . lang('info') . '</a>';
            ...
            <i class="fas fa-bars"></i> ' . lang('actions') . ' <span class="caret"></span>
                ...
                <ul class="dropdown-menu">
                    ...
                    ' . ($options5 ? '<li><a href="' . site_url("patient/delete?id=" . $patient->id) . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"> <i class="fa fa-trash"></i> ' . lang('delete') . ' </a></li>' : '') . '
    ...
    function getPatientPayments()
    {
        ...
        foreach ($data['patients'] as $patient) {
            ...
            $options4 = '<a class="btn btn-warning btn-sm invoicebutton" title="' . lang('payment') . ' ' . lang('history') . '" href="finance/patientPaymentHistory?patient=' . $patient->id . '"><i class="fa fa-money-bill-alt"></i> ' . lang('payment') . ' ' . lang('history') . '</a>';
