...
class Appointment extends MX_Controller
{
    ...
    function getAppointmentByJason()
    {
        ...
        foreach ($query as $entry) {
            ...
            $info = lang('status') . ': ' . $appointment_status . '<br>' . lang('patient') . ': ' . $patient_name . '<br/>' . lang('phone') . ': ' . $patient_mobile . '<br/> Doctor: ' . $doctor . '<br/>' . lang('remarks') . ': ' . $entry->remarks;
            if ($entry->status == 'Pending Confirmation') {
                $color = 'yellowgreen';
                $textColor = '#333333';
            }
    ...
    function getAppoinmentList()
    {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $option1 = '<button type="button" class="btn btn-primary btn-sm btn_width editbutton" data-toggle="modal" data-id="' . $appointment->id . '"><i class="fa fa-edit"> ' . lang('') . '</i></button>';
            ...
            $option2 = '<a class="btn btn-info btn-sm btn-danger delete_button" href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"> </i></a>';
            ...
            <i class="fas fa-bars"></i> ' . lang('actions') . ' <span class="caret"></span>
                ...
                <ul class="dropdown-menu">
                    ' . ($option1 ? '<li><a  class="editbutton" data-toggle="modal" data-id="' . $appointment->id . '">  <i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('') . '</a></li>' : '') . '
                    ' . ($option2 ? '<li><a href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"> <i class="fa fa-trash"></i> ' . lang('delete') . ' </a></li>' : '') . '
                    ' . ($options7 ? '<li><a href="meeting/instantLive?id=' . $appointment->id . '" target="_blank" onclick="return confirm(\'Are you sure you want to start a live meeting with this patient? SMS and Email will be sent to the Patient.\');"> <i class="fa fa-headphones"></i> ' . lang('start_live') . ' </a></li>' : '') . '
                </ul>
            </div>';
    ...
    function getUpcomingAppoinmentList()
    {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if ($appointment->date > strtotime(date('Y-m-d'))) {
                ...
                <i class="fas fa-cogs"></i> ' . lang('actions') . ' <span class="caret"></span>
            </button>
                    <ul class="dropdown-menu">
                        ' . ($option1 ? '<li><a  class="editbutton" data-toggle="modal" data-id="' . $appointment->id . '">  <i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('') . '</a></li>' : '') . '
                        ' . ($option2 ? '<li><a href="appointment/delete?id=' . $appointment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"> <i class="fa fa-trash"></i> ' . lang('delete') . ' </a></li>' : '') . '
